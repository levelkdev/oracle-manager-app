const TimeMedianDataFeedOracle = artifacts.require('TimeMedianDataFeedOracle')
...
contract('TimeMedianDataFeedOracle', (accounts) => {
  const dataSource = accounts[1]
  let dataFeed

  before(async ()=> {
    dataFeed = await TimeMedianDataFeedOracle.new()
  ...
  describe('medianizeByTimeframe', () => {
    ...
    it('reverts if startDate is after endDate', async () => {
      return assertRevert(async () => {
        await dataFeed.medianizeByTimeframe.call(DATE_2, DATE_1)
      })
    })

    it('reverts if all recorded dates are after endDate', async () => {
      return assertRevert(async () => {
        await dataFeed.medianizeByTimeframe.call(DATE_10 + 60, DATE_10 + 60 * 60)
      })
    })
